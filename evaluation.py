...
SCHEMA_ERROR = 4
TEXT_ERROR = 5
CODE_INFO = ['success', 'file_reading_error', 'encoding_error', 'json_parse_error', 'schema_error', 'input_text_not_in_dataset']
        ...
        for line in predict_file_zip.open(predict_file):
            ...
            for spo_item in json_info['spo_list']:
                if type(spo_item) is not dict or 'subject' not in spo_item \
                        or 'predicate' not in spo_item or 'object' not in spo_item:
                    ret_code = SCHEMA_ERROR
                    return predict_result, ret_code
...
if __name__ == '__main__':
    ...
    golden_filename = "evaluation_data/dev_data/dev_data.json"
    predict_filename = "evaluation_data/result_not_keep_empty_spo_list_convert.zip"
    location_filename = ""
    alias_filename = ""
